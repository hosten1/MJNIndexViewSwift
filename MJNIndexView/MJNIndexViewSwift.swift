...
//   MARK: ---    calculating initial values and sizes then setting them
    fileprivate func getAllItemsSize(){
        ...
        let capitalLetterHeight:Float
        let ascender:Float
        let descender:Float
        var entireHeight:Float
        if self.selectedItemFont != nil {
            lineHeight = Float(self.font.lineHeight)
            maxlineHeight = Float(selectedItemFont.capHeight)
            capitalLetterHeight = Float(self.font.capHeight)
            ascender = Float(self.font.ascender)
            descender = -Float(self.font.descender)
            entireHeight = ascender
            ...
            if (self.checkForLowerCase() && self.checkForUpperCase()) {
                ...
                maxlineHeight = Float(selectedItemFont.lineHeight)
            } else if (self.checkForLowerCase() && !self.checkForUpperCase()) {
                entireHeight = capitalLetterHeight + descender;
                maxlineHeight = Float(selectedItemFont.lineHeight)
            }
        }else{
            print("hosten unwrapping error \(String(describing: self.selectedItemFont))  at lines 329s")
            lineHeight = Float(self.font.lineHeight)
            maxlineHeight = 20
            capitalLetterHeight = 10
            ascender = Float(self.font.ascender)
            descender = -Float(self.font.descender)
            entireHeight = ascender
        }
    ...
    fileprivate func positionForIndexItemsWhilePanLocation(location : CGPoint){
        ...
        for itemAttribute in self.itemsAtrributes {
            ...
            if let actualRangeOfDeflectionT = self.actualRangeOfDeflection{
                ...
                let mappedAmplitude:Float = self.maxItemDeflection / self.itemsOffset / Float(actualRangeOfDeflectionT)
                
                // now we are checking if touch is within the range of items we would like to deflect
                let min:Bool = Float(location.y) > Float(point.y) - (Float(actualRangeOfDeflectionT) * self.itemsOffset)
                let max:Bool = Float(location.y) < Float(point.y) + (Float(actualRangeOfDeflectionT) * self.itemsOffset)
                
                if (min && max) {
                    
                    // these calculations are necessary to make our deflection not linear
                    let differenceMappedToAngle:Float = 90.0 / (self.itemsOffset * Float(actualRangeOfDeflectionT));
                    ...
                    let differenceMappedToRange:Float = Float(actualRangeOfDeflectionT) / (Float(actualRangeOfDeflectionT) * self.itemsOffset);
                    
                    let zPosition:Float = Float(actualRangeOfDeflectionT) - Float(fabs(point.y - location.y)) * differenceMappedToRange;
                    ...
                    let differenceMappedToColorChange:Float = 1.0 / (Float(actualRangeOfDeflectionT) * self.itemsOffset);
                    ...
                    if (selectedInRange || firstItemInRange || lastItemInRange) {
                        ...
                        if (!self.getSelectedItemsAfterPanGestureIsFinished && Int(self.section!) != section) {
                            if let sectionTitle = self.indexItems?[section] {
                                dataSource?.swiftSectionFor(SectionMJNIndexTitle: sectionTitle, atIndex: section)
...
//        }
    ...
    fileprivate func  drawIndex(){
        ...
        for itemAttribute in self.itemsAtrributes {
            ...
            if let currentFontT = itemAttribute.object(forKey: "font" as AnyObject){
                currentFont = currentFontT as! CTFont
                
                var singleItemTextLayer:CATextLayer = CATextLayer.init()
                
                if let laye = itemAttribute.object(forKey: "layer" as AnyObject) {
...
//        }
        ...
        if self.curtain {
            ...
            let multiplier:Float = Float(2.0);
